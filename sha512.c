...
void sha512_finalize(sha512_ctx_t *ctx, uint8_t *dst) {
  ...
  *p = 0x80; p++;
  ...
  q += sizeof(uint32_t) * 3;
  memset(p, 0x00, q - p);
  dumpbe32(q, ctx->len << 3);
  sha512_chunk(ctx->s, ctx->buf, SHA512_BLOCK_SIZE);

  dumpbe64(dst, ctx->s[0]);
  dumpbe64(dst + 8, ctx->s[1]);
  dumpbe64(dst + 16, ctx->s[2]);
  dumpbe64(dst + 24, ctx->s[3]);
  dumpbe64(dst + 32, ctx->s[4]);
  dumpbe64(dst + 40, ctx->s[5]);
  dumpbe64(dst + 48, ctx->s[6]);
  dumpbe64(dst + 56, ctx->s[7]);
}
